  ...
  if (canvas_left.getContext) {
      ...
      switch (this.shape) {
        ...
        case "ellipse":
          ...
          context_right = null;
          context_left.beginPath();
          context_left.ellipse(canvas.canvas_width/2, canvas.canvas_height/2, canvas.width/2, 90, 0, 0, Math.PI*2);
          context_left.stroke();

          // 焦点
          var marginX = (canvas.canvas_width - canvas.width)/2;
          context_left.beginPath();
          var f = Math.sqrt(Math.pow(canvas.width/2, 2) - Math.pow(90, 2));
...
function drawEllipse() {
  ...
  var count = 0;
  var marginX = new BigNumber(canvas.canvas_width).minus(canvas.width).div(2);
  var marginY = new BigNumber(canvas.canvas_height).div(2).minus(new BigNumber(90));
  var posX = new BigNumber(marginX).plus(new BigNumber(start_position).times(canvas.width));
  var posY = new BigNumber(canvas.canvas_height).div(2);
  var rad = new BigNumber(angle).div(180).times(Math.PI);
  var moveX = Math.cos(rad);
  var moveY = Math.sin(rad);

  var a = new BigNumber(canvas.width).div(2); // 長径
  var b = new BigNumber(90); // 短径
  var f = Math.sqrt(Math.pow(canvas.width/2, 2) - Math.pow(90, 2)); // 焦点
